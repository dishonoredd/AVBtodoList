  ...
  return completedTodos.length ? (
    <ul className="mt-4 flex flex-col gap-2 ">
      {completedTodos.map((todo) => (
        <li
          key={todo.id}
          className={`duration-200 p-5 rounded-lg flex items-center justify-between flex-wrap min-h-fit w-full 
           break-all shadow-md ${darkmode ? "bg-gray-700" : "bg-white"}`}
        >
          <p
            className={`duration-200 max-w-[75%] ${
          ...
          <div className="flex items-center right-0 gap-3">
            <p className={`duration-200 ${darkmode && "text-gray-300"}`}>
              {props.curDate}
            </p>
            <button
              onClick={() => {
              ...
              className={`duration-200 cursor-pointer h-full rounded-lg ${
    ...
    <div className="grow-1 flex items-center justify-center">
      <p
        className={`duration-200 text-2xl opacity-50 ${
